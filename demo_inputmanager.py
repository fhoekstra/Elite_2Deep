...
class InputManager:
    def __init__(self, joystick_idx=1):
        ...
        # If you would like there to be a keyboard fallback configuration,
        # fill those out here in this mapping. If you wanted the keyboard keys
        # to be configurable, you could probably copy the same sort of system
        # I use for the joystick configuration for the keyboard.
        # But that's getting fancy for a simple tutorial.
        self.key_map = {
            pg.K_UP: 'up',
            pg.K_DOWN: 'down',
            pg.K_LEFT: 'left',
        ...
        # Quitting the window is raised as an input event. And typically you
        # also want that event raised when the user presses escape which is not
        # something you want to configure on the joystick. That's why it's
        # wired separately from everything else. When escape is pressed or the
        # user closes the window via its chrome, this flag is set to True.
    ...
    def get_events(self, pgevents=None):
        ...
        for button in self.buttons:
            ...
            # determine what something like "Y" actually means in terms of
            # the joystick
...
class MyJoystickWrapper:
    ...
    def configure_button(self, button):
        ...
        for hat_index in range(js.get_numhats()):
            ...
            if hat_status[0] < -.5 \
                    and not self.is_hat_used(hat_index, 'x', -1):
                ...
                return True
            if hat_status[1] < -.5 \
                    and not self.is_hat_used(hat_index, 'y', -1):
        ...
        for ball_index in range(js.get_numballs()):
            ...
            if ball_status[0] < -.5 \
                    and not self.is_ball_used(ball_index, 'x', -1):
                self.joystick_config[button] = ('is_ball', ball_index, 'x', -1)
                return True
            elif ball_status[0] > .5 \
                    and not self.is_ball_used(ball_index, 'x', 1):
                self.joystick_config[button] = ('is_ball', ball_index, 'x', 1)
                return True
            if ball_status[1] < -.5 \
                    and not self.is_ball_used(ball_index, 'y', -1):
                self.joystick_config[button] = ('is_ball', ball_index, 'y', -1)
                return True
            elif ball_status[1] > .5 \
                    and not self.is_ball_used(ball_index, 'y', 1):
                self.joystick_config[button] = ('is_ball', ball_index, 'y', 1)
                return True

        # check axes for activity...
        # (that's plural of axis. Not a tree chopping tool.
        # Although a USB Axe would be awesome!)
    ...
    def is_button_used(self, button_index):
        for button in self.buttons:
            ...
            if config is not None and config[0] == 'is_button' \
    ...
    def is_hat_used(self, hat_index, axis, direction):
        for button in self.buttons:
            ...
            if config is not None and config[0] == 'is_hat':
                if config[1] == hat_index \
                        and config[2] == axis and config[3] == direction:
                    return True
        return False
    ...
    def is_ball_used(self, ball_index, axis, direction):
        for button in self.buttons:
            config = self.joystick_config.get(button)
            if config is not None and config[0] == 'is_ball':
                if config[1] == ball_index \
                        and config[2] == axis and config[3] == direction:
                    return True
        return False
