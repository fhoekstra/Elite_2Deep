...
class WpnRailgun(object):
    ...
    def _calc_dps(self):
        ...
        self.actual_dps = (  # including reloading
            (self.instant_dps * self.chargetime * self.clipsize)  # dmg
            / (self.clipsize * self.chargetime + self.reloadtime))  # time
    ...
    def _fire(self, shiplist, statlist):
        self.clip -= 1
        railbeam = ProjRailgun(self, self.mother.x, self.mother.y,
                               self.mother.phi, self.range)
...
class WpnPulseLaser(WpnBeamLaser):
    def __init__(self, mother, wpn_idx=0):
        ...
        self.chargetimer = None
        self.speed = 5000
        # del self.hittime, self.dps, self.heattimer, self.heatps
    ...
    def fire(self, objlist):
        self._heat()
        pulse = ProjPulseLaser(self, self.mother.x, self.mother.y,
                               self.mother.phi)
...
class ProjPulseLaser(KineticObject):
    ...
    def draw(self, surf, camparams):
        if self.timer.get() > self.flighttime or self.hp < 0:
            ...
            shapetodraw = shapetodraw + np.array([self.x, self.y])  # add pos
...
class WpnKineticRocket(object):
    ...
    def _calc_dps(self):
        ...
        self.actual_dps = (
                           0.0008 * 0.1 * self.rocketmass
                           * (self.mother.vmax + self.speed) ** 2
                          ) / self.reloadtime
    ...
    def _fire(self, shiplist, objlist, statlist):
        self.clip -= 1
        rocket = ProjKineticRocket(
            self, self.mother.x, self.mother.y, self.mother.phi)
...
class ProjKineticRocket(KineticObject):
    ...
    def collide(self, other, k=None):
        if self.timer.get() > self.armtime:
            ...
            self.hp = -1
            # only set ishit for hitmarkers if armed
            if hasattr(other, 'ishit'):
                other.ishit = 10
                other.hitbycolor = self.launcher.mother.color
            if k is None:
    ...
    def draw(self, surf, camparams):
        ...
        if self.timer.get() > self.flighttime or self.hp < 0:
            ...
            # rotate shape
            shapetodraw = rotate(self.shape, self.phi)
            # add physics position
            shapetodraw = shapetodraw + np.array([self.x, self.y])
            shapetodraw = xyworldtoscreen(shapetodraw, camparams)
            pg.draw.polygon(surf, self.color, shapetodraw, 0)
            return True  # keep after this draw cycle
