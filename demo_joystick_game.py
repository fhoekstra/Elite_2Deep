...
from demo_inputmanager import InputManager
...
def main():
    ...
    print("Plug in a USB gamepad. Do it! Do it now! Press enter after you"
          " have done this.")
    wait_for_enter()
    ...
    while not input_manager.quit_attempt:
        ...
        # In the first phase, the user will be prompted to configure the
        # joystick by pressing the key that is indicated on the screen
        # You would probably do this in an input menu in your real game.
        if not is_configured:
            success = configure_phase(screen,
                                      input_manager.buttons[button_index],
                                      input_manager)
        ...
        # In the second phase, the user will control a "character" on the
        # screen (which will be represented by a simple blue ball) that obeys
        # the directional commands, whether it's from the joystick or the
        # keyboard.
        else:
...
def configure_phase(screen, button, input_manager):
    ...
    # configure_button looks at the state of ALL buttons pressed on the
    # joystick and will map the first pressed button it sees to the current
    # button you pass in here.
